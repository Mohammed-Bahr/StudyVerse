---
title: Agent Skills
source: https://claude.nagdy.me/learn/skills/
author:
published:
created: 2026-05-09
description: Build reusable skills that Claude invokes automatically, with progressive loading, dynamic context, and subagent execution.
tags:
  - Claude
  - Opencode
  - Knowledge
...
[← Commands](4.%20Commands%20in%20Depth.md) | 🏠 [Home](../Home.md) | [Hooks →](6.%20Hooks.md)

---

> [!tip] 🔀 Opencode Equivalent
> - **Skills location:** `.opencode/skills/<name>/SKILL.md` (project) or `~/.config/opencode/skills/` (global) — Opencode also reads `.claude/skills/` for compatibility
> - **`SKILL.md` format:** Compatible between both tools — same YAML frontmatter + markdown body structure
> - **Dynamic context (`!command`):** Supported in Opencode (same `!` shell execution syntax)
> - **Invocation:** Skills appear as slash commands in both tools; in Opencode they're discovered via the `skill` tool
> - **Namespace:** In Opencode, skills from plugins use `plugin-name:skill-name` namespace (same as Claude Code)
> - **`disable-model-invocation`:** Not confirmed in Opencode — test before relying on it
> - **`context: fork`:** Opencode supports subagent isolation (separate context windows) but the frontmatter field may differ
> - **`argument-hint`:** Supported in both
> - **Legacy commands:** `.opencode/commands/*.md` works similarly to `.claude/commands/*.md`

Build reusable skills that Claude invokes automatically, with progressive loading, dynamic context, and subagent execution.
