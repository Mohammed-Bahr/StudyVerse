---
title: "Subagents"
source: "https://claude.nagdy.me/learn/subagents/"
author:
published:
created: 2026-05-09
description: "Delegate complex tasks to specialized AI agents with isolated context windows and custom system prompts."
tags:
  - Claude
  - Opencode
  - "#Claude/Subagents"
...
[← MCP](7.%20Model%20Context%20Protocol%20(MCP).md) | 🏠 [Home](../Home.md) | [Advanced Features →](9.%20Advanced%20Features.md)

---

> [!tip] 🔀 Opencode Equivalent
> - **Agent files location:** `.opencode/agents/` (project) or `~/.config/opencode/agents/` (global) — same markdown + YAML frontmatter format
> - **Creating agents:** `opencode agent create` — interactive wizard to scaffold a new agent definition
> - **Built-in agents:** Opencode ships `@general` (broad tasks) and `@explore` (read-only analysis) — similar to Claude Code's `general-purpose` and `Explore`
> - **Frontmatter format:** Compatible — `name`, `description`, `model`, `tools` fields work in both
> - **Worktree isolation:** `isolation: worktree` — supported concept in Opencode (check current docs for exact frontmatter key)
> - **Memory:** Opencode agents can have persistent memory via `AGENTS.md` in the agent's scope
> - **Chaining agents:** Both tools support sequential agent chaining via natural language
> - **Agent Teams (experimental):** This is Claude Code-specific (`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`); Opencode supports multiple parallel sessions natively instead
> - **`claude --agent <name>`:** Equivalent is `opencode --agent <name>` (check current Opencode CLI docs)
...
`isolation: worktree` gives the agent its own git worktree and branch to make changes without touching your main working tree. When the agent finishes, it returns the worktree path and branch name for you to review and merge. If it made no changes, the worktree is cleaned up automatically. Agents in worktree isolation can also use the `ExitWorktree` tool to explicitly leave the worktree before the task ends — useful when the agent needs to hand off mid-task or signal completion early. The matching `EnterWorktree` tool creates a fresh isolated worktree and switches into it on demand; pass a `path` argument to jump into an existing worktree of the current repository instead of spinning up a new one, which is handy for resuming review across parallel branches without restarting the session.
...
Set `CLAUDE_CODE_FORK_SUBAGENT=1` to enable forked subagents. A forked subagent inherits the full conversation context from the main session instead of starting fresh. When enabled, `/fork` spawns a forked subagent rather than acting as an alias for `/branch`, and all subagent spawns run in the background. This is available in interactive mode only:
