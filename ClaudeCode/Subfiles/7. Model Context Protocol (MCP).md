...
source: "https://claude.nagdy.me/learn/mcp/"
author:
published:
created: 2026-05-09
description: "Connect Claude to external services, databases, and APIs using the Model Context Protocol."
tags:
  - Claude
  - Opencode
  - "#Claude/MCP"
...
[← Hooks](6.%20Hooks.md) | 🏠 [Home](../Home.md) | [Subagents →](8.%20Subagents.md)

---

> [!tip] 🔀 Opencode Equivalent
> Opencode has full MCP support with a similar but slightly different configuration:
> - **Adding servers:** Configure in `opencode.json` under `mcp:` key (instead of `~/.claude.json` / `.mcp.json`)
> - **MCP auth:** `opencode mcp auth <server>` CLI command for OAuth flows
> - **Config example in `opencode.json`:**
>   ```json
>   { "mcp": { "github": { "command": "npx", "args": ["@modelcontextprotocol/server-github"], "env": { "GITHUB_TOKEN": "${GITHUB_TOKEN}" } } } }
>   ```
> - **Tool discovery:** Opencode also defers MCP tools and discovers them on demand
> - **Concurrent connections:** Both tools connect MCP servers concurrently at startup
> - **Scopes:** Opencode has project-scope (`opencode.json`) and global-scope (`~/.config/opencode/opencode.json`) — no separate `.mcp.json` file
> - **`/mcp` command:** Not available in Opencode; use `opencode mcp auth` CLI instead
> - **Elicitation:** MCP elicitation (structured user input requests) is supported in Opencode
